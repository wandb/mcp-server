...
async def query_weave_traces_tool(
    ...
    try:
        ...
        await save_result_to_sandbox_if_requested(
            result=json_output_string, save_filename=save_filename, logger=logger
        )
...
async def query_wandb_tool(
    ...
    # Save to sandbox if requested
    await save_result_to_sandbox_if_requested(
        result=gql_result, save_filename=save_filename, logger=logger
    )
...
def cli():
    ...
    if _sandbox_available:
        ...
        if "pyodide" in _sandbox_types:
            ...
            try:
                from wandb_mcp_server.mcp_tools.code_sandbox.pyodide_sandbox import (
                    PyodideSandbox,
                )

                # Use synchronous pre-download to avoid event loop issues
                success = PyodideSandbox.pre_download_pyodide_sync()
                if success:
                    logger.info("Pyodide packages downloaded and ready for use")
                else:
                    logger.info(
                        "Pyodide pre-download completed with warnings - will retry on first use"
                    )
            except Exception as e:
                logger.warning(f"Failed to pre-download Pyodide: {e}")
                logger.info("Pyodide will download packages on first use")
    else:
