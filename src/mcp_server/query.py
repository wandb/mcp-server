...
from weave.trace_server.interface.query import Query, EqOperation, GtOperation, LiteralOperation, AndOperation, OrOperation, GetFieldOperator, ContainsOperation, ContainsSpec, NotOperation
...
def query_traces(
    ...
    sort = trace_server_interface.SortBy(
        field=sort_by,
    ...
    if filters:
        ...
        calls_filter = trace_server_interface.CallsFilter(trace_roots_only=trace_roots_only)
        ...
        if 'trace_ids' in filters:
            trace_ids = filters['trace_ids'] if isinstance(filters['trace_ids'], list) else [filters['trace_ids']]
        ...
        if 'parent_ids' in filters:
            parent_ids = filters['parent_ids'] if isinstance(filters['parent_ids'], list) else [filters['parent_ids']]
            calls_filter.parent_ids = parent_ids
        
        if 'call_ids' in filters:
            call_ids = filters['call_ids'] if isinstance(filters['call_ids'], list) else [filters['call_ids']]
        ...
        if isinstance(display_name, str):
            ...
            operations.append(
                ContainsOperation(**{
                    "$contains": ContainsSpec(
                        input=GetFieldOperator(**{"$getField": "display_name"}),
                        substr=LiteralOperation(**{"$literal": display_name.pattern}),
                        case_insensitive=True
                    )
                })
            )
        ...
        for attr_path, attr_value in attributes.items():
            ...
            operations.append(
                EqOperation(**{
                    "$eq": (
                        field_op,
                        LiteralOperation(**{"$literal": attr_value})
                    )
                })
            )
