...
class CheckCorrectness(BaseModel):
    ...
    reasoning: str = Field(..., description="Reasoning about the correctness of the tool response given \
...
expected outout should be clear to see in the response data.")
    is_correct: bool = Field(..., description="Whether the tool response is correct given the \
expected output.")
...
def call_anthropic(
    ...
    if client is None:
        raise EnvironmentError("ANTHROPIC_API_KEY environment variable must be set for live Anthropic calls.")
    if tools:
        return client.messages.create(model=model_name, max_tokens=4000, tools=tools, messages=messages)
        ...
        return client.messages.create(model=model_name, max_tokens=4000, messages=messages)
